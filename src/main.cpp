...
#include "types/gameoffsets/SDK/Engine_parameters.hpp"
#include "types/gameoffsets/SDK/Starbreeze_parameters.hpp"
...
bool VerifyGameVersion()
{
	...
	DWORD versionInfoSize = GetFileVersionInfoSizeA(szBlock, nullptr);
	if (versionInfoSize == 0){
		Utils::LogError("Failed to get version info size!");
		return false;
	}

	std::vector<BYTE> vecVersionData(versionInfoSize);

	// Read version info
	if (!GetFileVersionInfoA(szBlock, 0, versionInfoSize, vecVersionData.data())){
...
LONG WINAPI ExceptionHandler(PEXCEPTION_POINTERS pExceptionInfo)
{
	...
	ssStackTrace << std::endl << "Exception occurred at: 0x" << std::hex << pExceptionInfo->ExceptionRecord->ExceptionAddress << std::endl;
	ssStackTrace << "Exception code: 0x" << std::hex << pExceptionInfo->ExceptionRecord->ExceptionCode << std::endl;
